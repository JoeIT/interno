...
class Compra {
	...
	function insert_Compra($fecha,$nit,$razonsocial,$factura,$autorizacion,$codigo_control,$total_facturado,$ice,$exentos,$total_exentos,$iva,$cod_pg,$alfanumerico,$usuario_id)
	{
		$data = null;
		$sql = "INSERT INTO tcompra (fecha, nit, razon_social, factura, autorizacion, codigo_control, total_facturado,
				ice, exento, importe, credito_fiscal,cod_pg,usuario_id,fechareg,alfanumerico) VALUES ('$fecha', '$nit', '$razonsocial',
				'$factura', '$autorizacion', '$codigo_control', '$total_facturado','$ice','$exentos','$total_exentos','$iva','$cod_pg','$usuario_id',now(),'$alfanumerico');";		
	...
	function buscarcompra($codigo)
	{
		...
		$sql = "select cod_compra,fecha,nit,razon_social,factura,
				...
				exento,importe,credito_fiscal,cod_pg,usuario_id,fechareg 
	...
	function updateCompra($fecha,$nit,$razonsocial,$factura,$autorizacion,$codigo_control,$total_facturado,$ice,$exentos,$total_exentos,$iva,$cod_pg,$usuario_id,$codigo)
	{
		$sql = "UPDATE tcompra SET fecha='$fecha', nit='$nit', razon_social='$razonsocial', factura='$factura', autorizacion='$autorizacion', codigo_control='$codigo_control', total_facturado='$total_facturado', ice='$ice', exento='$exentos', importe='$total_exentos', credito_fiscal='$iva', usuario_id='$usuario_id', fechareg=now() WHERE cod_compra='$codigo';";
