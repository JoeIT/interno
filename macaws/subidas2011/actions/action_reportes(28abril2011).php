...
if(isset($_SESSION['usuario_id']))
{
	...
	if(	isset($_REQUEST['genrepcv']) ||	isset($_REQUEST['generar']))
	{
		...
		else
		{
			...
			if(	isset($usuario_id))
			{
				$paginacion = null;
				if(	$_POST['tipo']==0	)
				$paginacion = $report->calculatePagesReport($_POST['intervalo'],0,$_POST['gestion'],$periodo,$_POST['alfanumerico']);
				if(	$_POST['tipo']==1	)
				$paginacion = $report->calculatePagesReport($_POST['intervalo'],1,$_POST['gestion'],$periodo,$_POST['tipoventa']);
				if($_POST['tipo']==2)
				{
					
					$davinci = $report->reporteDaVinci($_POST['tipo'],$periodo,$_POST['gestion']);
	...
	if( isset($_GET['iniInt'])&&isset($_GET['endInt'])&&isset($_GET['tiporep'])&&isset($_GET['gestion'])&&isset($_GET['periodo']))
	{
		...
		$contribuyente = $consucur->obtenerContriSucursal();
		...
		{
			if(	$_REQUEST['alfa']!=2	)
			$result['res'] = $report->buscar_ComprasReporte("","razon_social",$_GET['gestion'],$_GET['periodo'],null,$_GET['iniInt'],$_GET['endInt']);
			else
			{
				$result['res'] = $report->buscar_ComprasAlfanumericoReporte("","razon_social",$_GET['gestion'],$_GET['periodo'],null,$_GET['iniInt'],$_GET['endInt']);
				$_SESSION['repcomalfalcv']="alfanumerico";
			}

			if($_GET['pag']==$_GET['ttl'])
			$totales = $report->CompraTotalesGenerales($_GET['gestion'],$_GET['periodo']);
			$_SESSION['tiporeplcv']=0;
		}
		if(	$_GET['tiporep']==1	)
		{

			$result['res'] = $report->buscar_VentasReportes("","razon_social",$_GET['gestion'],$_GET['periodo'],null,$_GET['iniInt'],$_GET['endInt'],$_GET['tvent']);
			if($_GET['pag']==$_GET['ttl'])
			$totales = $report->ventaTotalesGenerales($_GET['gestion'],$_GET['periodo'],$_GET['tvent']);
