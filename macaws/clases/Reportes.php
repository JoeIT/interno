...
{$sql = "select tv.*,tpg.gestion,tpg.periodo,tpg.estado from tventa as tv, tgestion as tg, tperiodogestion as tpg
	...
	function buscar_ComprasReporte($texto_search,$criterio,$gestion,$periodo,$fec,$ini,$fin,$sucursal)//reporte del comprar desde interfaz reporte
	{
		$data = null;
		$i=0;
    ...
    if(strcmp($sucursal,"todos")==0)
    {
        $sql = "select tv.*,tpg.gestion,tpg.periodo,tpg.estado 
				from tventa as tv, tgestion as tg, tperiodogestion as tpg
				where	tg.gestion = tpg.gestion and
						tv.cod_pg = tpg.cod_pg 
						$text_gestion
						$text_periodo 
						$text_fecha 
						$text_criterio 
						$tipodeventa
				order by tv.fecha,  cast(tv.factura as UNSIGNED)
				$limite;";
    }
	/******************************* c:\wamp\www\sistema\macaws\clases\Reportes.php ******************************************/
    else
        $sql = "select tv.*,tpg.gestion,tpg.periodo,tpg.estado 
				from tventa as tv, tgestion as tg, tperiodogestion as tpg
				where 	tg.gestion = tpg.gestion and
						tv.cod_pg = tpg.cod_pg and
						tv.sucursal_id = '$sucursal'  
						$text_gestion 
						$text_periodo 
						$text_fecha 
						$text_criterio 
		...
		while($row = mysql_fetch_array($res))
		{
			...
			$data[$i]['credito_fiscal']=$this->truncarCantidad($row['credito_fiscal']);
			$data[$i]['cod_pg']=$row['cod_pg'];
			$data[$i]['usuario_id']=$row['usuario_id'];//echo "<>".$row['usuario_id'];
            $data[$i]['sucursal_id']=$row['sucursal_id'];//echo "+".$row['sucursal_id'];
			$data[$i]['fecha_reg']=$row['fechareg'];
			$data[$i]['periodo']=$row['periodo'];
			$data[$i]['gestion']=$row['gestion'];
			$data[$i]['estado']=$row['estado'];
...
//	function CompraTotalesGenerales($gestion,$periodo)
	...
	function calculatePagesReport($intervalo,$tipo,$gestion,$periodo,$tipovc=1,$sucursal)//sucursal=> todos (consolidado), sucursal[i] (especifico)
	{
		$data = 0;
		$sql=null;
		$paginas = null;
	...
	//       buscar_ComprasAlfanumericoReporte("","razon_social",$_GET['gestion'],$_GET['periodo'],null,$_GET['iniInt'],$_GET['endInt'],$_GET['sucursal']);
...
//VOLVER A LO ANTERIOR PERO NO SE RECIBE NINGUN VALOR EN EL COD ID DE LA SUCURSAL //and tc.sucursal_id = '".$sucursal."'
/*$sql = "select tc.*,tpg.gestion,tpg.periodo,tpg.estado from tcompra as tc, tgestion as tg, tperiodogestion as tpg
				where
				tg.gestion = tpg.gestion 
				and tc.cod_pg = tpg.cod_pg and tc.sucursal_id = '$sucursal' order by tc.fecha;";*/
        }
		//echo "$sql<br>";
		$res = mysql_query($sql,$this->link);
		while($row = mysql_fetch_array($res))
	...
	function reporteDaVinci($tipo,$periodo,$gestion,$sucursal)
	{
		...
		while($row = mysql_fetch_array($result))
		{
		 ...
		 if($tipo==3)
    	 {
			...
			$data[$i]['importe']=$row['importe'];
			$data[$i]['credito_fiscal']=$row['credito_fiscal'];
            $data[$i]['codigo_control']=$row['codigo_control'];//se encuentra asi en el servidor pero hasta antes del 26/abril/2011 se encontraba comentado en el local
			$data[$i]['sucursal']=$row['sucursal_id'];		
			$i++;
