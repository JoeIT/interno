...
class Venta {
	...
	function insert_Venta($fecha,$nit,$razonsocial,$factura,$autorizacion,$codigo_control,$total_facturado,$ice,$exentos,$total_exentos,$iva,$cod_pg,$usuario_id,$tipo,$sucursal_id)
	{
		$data = null;
		$sql = "INSERT INTO tventa (fecha, nit, razon_social, factura, autorizacion, codigo_control, total_facturado,
				ice, exento, importe, credito_fiscal,cod_pg,usuario_id,fechareg,tipo,sucursal_id) VALUES ('$fecha', '$nit', '$razonsocial',
				'$factura', '$autorizacion', '$codigo_control', '$total_facturado','$ice','$exentos','$total_exentos','$iva','$cod_pg','$usuario_id',now(),'$tipo','$sucursal_id');";				
		//echo $sql;
	...
	function buscarVenta($codigo)
	{
		...
		$sql = "select cod_venta,fecha,nit,razon_social,factura,
				...
				exento,importe,credito_fiscal,cod_pg,usuario_id,fechareg,tipo,sucursal_id 
	...
	function updateVenta($fecha,$nit,$razonsocial,$factura,$autorizacion,$codigo_control,$total_facturado,$ice,$exentos,$total_exentos,$iva,$cod_pg,$usuario_id,$tipoventa,$codigo,$sucursal)
	{
		$sql = "UPDATE tventa SET fecha='$fecha', nit='$nit', razon_social='$razonsocial', factura='$factura', autorizacion='$autorizacion', codigo_control='$codigo_control', total_facturado='$total_facturado', ice='$ice', exento='$exentos', importe='$total_exentos', credito_fiscal='$iva', usuario_id='$usuario_id', fechareg=now(),tipo='$tipoventa',sucursal_id='$sucursal' WHERE cod_venta='$codigo';";
		mysql_query($sql,$this->link);
	}
