	...
	if (!empty($_POST)){
		...
		if ($_POST['guardar']){
			...
			*/
			...
			//fotografia
			if($HTTP_POST_FILES['fotografia']['name'] != ""){
				$nombre_archivo = $HTTP_POST_FILES['fotografia']['name'];
				$tipo_archivo = $HTTP_POST_FILES['fotografia']['type'];
				$tamano_archivo = $HTTP_POST_FILES['fotografia']['size']; 
			...
			if (isset($error)){
				...
				if ($flag_imagen == true){
					...
					move_uploaded_file($HTTP_POST_FILES['fotografia']['tmp_name'], "../../fotografia/".$nombre_archivo);
