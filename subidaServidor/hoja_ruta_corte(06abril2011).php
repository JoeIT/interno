...
//******************** funciones generales basadas en dividido ********************//
 ...
 //*******************************************
      ...
      function faltantes_cuero()
    {
        $con = new DBmanejador();
		if($con->conectar() == true){
      $consulta = "SELECT o.orden_id,count(h.hoja_id) as faltan FROM `tdetalleordenesproduccion` as tdo,hoja as h,tordenesproduccion o WHERE o.orden_id=tdo.orden_id and tdo.detalle_id = h.detalle_id  and 
            h.hoja_id not in (SELECT cu.hoja_id FROM `cuero_utilizado` as cu,hoja as h,tordenesproduccion o WHERE o.orden_id=cu.orden_id and  cu.hoja_id = h.hoja_id )GROUP BY o.orden_id
            ORDER BY o.fecha DESC, o.cup_num DESC
		    LIMIT 0 ,71";
    ...
    function lista_cuero_utilizados($orden_id)
    {
		...
		if($con->conectar()==true)
		{
            ...
            $consulta="SELECT cu.hoja_id,tdo.orden_id,tdo.pedido,cu.fecha_corte,concat(nombres,' ',apellidos) as cortador,f.nombre_familia,e.nombre_estilo,h.cantidad,tcu.descripcion as cuero,tco.descripcion as color,cu.cuero_utilizado, cu.retaceria,cu.retazos,cu.desperdicio,cu.cuero_utilizado_pies  
