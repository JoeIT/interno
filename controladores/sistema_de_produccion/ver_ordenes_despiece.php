...
if(!isset($_SESSION['logeo'])){
	...
	if($funcion=="detalle"){
		$id=$_GET['elegido'];
		$detalle=$detalle_orden->obtener_detalle_orden_despieces($id);
		...
		//******la siguiente linea permite q almacen pueda ver el despiece sin q pueda modificar el despiece permite generar una bandera
		 $smarty->assign('id',$_SESSION['grupo_id']);
		...
		if($funcion=="buscar"){
			...
			if (isset($error)){
				...
				$lista= $ordenes->consulta_lista_ordenes_despiece();
				$smarty->assign('ordenes',$lista);
				$smarty->assign('nombres', $_SESSION["nombres"]);
				$smarty->assign('apellidos', $_SESSION["apellidos"]);
				$smarty->assign('errores',$error);
				$smarty->assign('tabu',$_SESSION['tabu']);
				$smarty->display('sistema_de_produccion/despiece/lista_ordenes_despiece.html');
			} else {
				$consulta = $ordenes->consultar_busqueda_despiece($cadena,$opcion);
				if ($consulta != null){
