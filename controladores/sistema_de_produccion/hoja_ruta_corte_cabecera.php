	...
	if(isset($_GET["busqueda_ajax"])){    
		...
		switch ($opcion){
			...
			case 3:{
                ...
                $nro_instructivo = $hoja_ruta_corte->instructivo_asignado($hid);
                
				$smarty->assign('cabecera', $cabecera);
                $smarty->assign('instructivo', $nro_instructivo);
                ...
                $hid = $_GET['hid'];
                //============================================================    
                $despiece = $hoja_ruta_corte->despiece_completo($hid);
                if($despiece[despiece]==1)//DESPIECE COMPLETO
                {
                    $cabecera = $hoja_ruta_corte->cabecera_reporte($hid);
                    $smarty->assign('cabecera', $cabecera);
                    ...
                    $lista_corte=$hoja_ruta_corte->detalle_corte_rg27($hid);
                    $smarty->assign('lista_corte', $lista_corte);
                    $smarty->display('sistema_de_produccion/corte/hoja_ruta_reporte.html');
                }
                else//DESPIECE INCOMPLETO
                {
                echo "El despiece no a sido completado...!";
                }
                break;
			}
			case 4:{
