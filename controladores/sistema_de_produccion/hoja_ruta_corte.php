...
if(!isset($_SESSION['logeo'])){
	...
	if(isset($_GET["busqueda_ajax"])){
	   ...
	   //********************************************
       //inicio de guardar valores de cuero utilizado
       //********************************************
		if ($_GET['funcion'] == "validar")
		{
			foreach($_POST['hoja_id'] as $indice => $valor)
			{
				if($_POST['fecha_corte'][$indice]!='' && $_POST['maquinista_id'][$indice]!='' && $_POST['cuero_utilizado'][$indice]!='' && $_POST['retaceria'][$indice]!='' && $_POST['retazos'][$indice]!='' && $_POST['desperdicio'][$indice]!='' && $_POST['cuero_utilizado_pies'][$indice]!='')
					$ver=$hoja_ruta_corte->ingresar_cueroutilizado($valor,$_POST['orden_id'],$_POST['fecha_corte'][$indice],$_POST['maquinista_id'][$indice],$_POST['cuero_utilizado'][$indice],$_POST['retaceria'][$indice],$_POST['retazos'][$indice],$_POST['desperdicio'][$indice],$_POST['cuero_utilizado_pies'][$indice],$_POST['unidad'][$indice]);      
			}
			
			$cabecera = $ordenes->obtener_cabecera_orden($_POST['orden_id']);
			$smarty->assign('cabecera', $cabecera);
			$detalles_id=$hoja_ruta_corte->verificando_registro($_POST['orden_id']);
			
			if (!isset($detalles_id))
				$band=1;
			
			$detalle=$hoja_ruta_corte->lista_cuero_utilizados($_POST['orden_id']);
			$smarty->assign('band', $band);
			$smarty->assign('detalle', $detalle);
			$smarty->display('sistema_de_produccion/corte/lista_cuero_utilizado.html');
		}
...
//
            ...
            case 10:{
				...
				$lista = $hoja_ruta_corte->consulta_lista_ordenes_anual(date("2012"));
                $faltantes = $hoja_ruta_corte->faltantes_cuero();
                foreach ($lista as $indice => $valor) {
                    $posicion=$lista[$indice]['orden_id'];
                    $lista[$indice]['falta']=$faltantes[$posicion];
                }
				$smarty->assign('ordenes', $lista);
				$smarty->display('sistema_de_produccion/corte/cuero_utilizado.html');
				break;
                }
            // Cuero utilizado por item
			case 11:{
                set_time_limit(100);
				$op_id = $_GET['oid'];
				$smarty->assign('op_id', $op_id);
                ...
                $detalles_id=$hoja_ruta_corte->verificando_registro($op_id);
                if (isset($detalles_id))
                {
					$detalle = $hoja_ruta_corte->detalle_orden_cuero($op_id, $detalles_id);
					$smarty->assign('detalle', $detalle);
					$smarty->display('sistema_de_produccion/corte/cuero_utilizado_detalle.html');
                }
                else
                { 
                    $detalle=$hoja_ruta_corte->lista_cuero_utilizados($op_id);
                    $band=1;
                    $smarty->assign('detalle', $detalle);
                    $smarty->assign('band', $band);
                    $smarty->display('sistema_de_produccion/corte/lista_cuero_utilizado.html'); 
			...
			case 13:{
				...
				$data = $hoja_ruta_corte->get_order_detail_leather($orderId);
				
				$smarty->assign('header', $ordenes->obtener_cabecera_orden($orderId));
				$smarty->assign('detail', $data);
				$smarty->display('sistema_de_produccion/corte/cuero_utilizado_orden_detalle.html');
