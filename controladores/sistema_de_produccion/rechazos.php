...
class Rechazo{
	...
	function resumen_rechazo_actualizar_id($rechazo_id)
	{
		...
		{
		    $consulta = "
			SELECT r.rechazo_id, r.asignacion_id, r.fecha_rechazo, CONCAT(p.apellidos, ' ', p.nombres ) AS responsable_fallo, 	
			r.cantidad, r.area_responsable, r.detalle_fallo, CONCAT(p1.apellidos, ' ', p1.nombres ) AS responsable_arreglo,r.clasificacion_fallo_id
			FROM trechazo r
			LEFT JOIN personal p ON r.responsable_id = p.personal_id, tarreglos a
			LEFT JOIN personal p1 ON a.responsable_arreglo = p1.personal_id
			WHERE r.rechazo_id = a.rechazo_id
			...
			if (!$resultado)
				return false;
					...
					$respuesta["descripcion_fallo"] = trim($row['detalle_fallo']);
