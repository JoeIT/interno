...
//******************** funciones generales ********************//
	...
	function instructivo_asignado($hid)
	{
		...
		if($con->conectar()==true)
		{
			...
			$resultado=mysql_query($consulta) or die('La consulta -instructivo- fall&oacute;: ' . mysql_error());
...
//SEPARANDO INSTRUCTIVO
/*Devuelve el despiece completo o no para este pedido*/
function despiece_completo($hid)
    {
        $con = new DBmanejador();
        if($con->conectar()==true)
        {
            $consulta= "SELECT    tdop.despiece
                        FROM    `hoja` h, `tdetalleordenesproduccion` tdop, `tordenesproduccion` top,familia f
                        WHERE    h.detalle_id = tdop.detalle_id
                        AND tdop.orden_id = top.orden_id
                        AND tdop.familia_id = f.familia_id
                        AND h.hoja_id=".$hid;
            ...
            $resultado=mysql_query($consulta) or die('La consulta -despiece completo- fall&oacute;: ' . mysql_error());
        ...
        if (!$resultado) return false;
            ...
            {    
                while($row = mysql_fetch_array($resultado))
                {
                            $respuesta["despiece"]= $row['despiece'];
                }
                return $respuesta;
            }
        }
...
//******************** funciones generales basadas en dividido ********************//
 ...
 //*******************************************
    ...
    function faltantes_cuero()
    {
		...
		$consulta = "
			SELECT o.orden_id,
				count(h.hoja_id) AS faltan 
			FROM tdetalleordenesproduccion tdo, 
				hoja h, 
				tordenesproduccion o 
			WHERE o.orden_id = tdo.orden_id 
				AND tdo.detalle_id = h.detalle_id 
				AND h.hoja_id not in (SELECT cu.hoja_id 
										FROM cuero_utilizado cu,
											hoja h,tordenesproduccion o 
										WHERE o.orden_id=cu.orden_id 
											AND cu.hoja_id = h.hoja_id ) 
			GROUP BY o.orden_id 
            ORDER BY o.fecha DESC, o.cup_num DESC 
		    LIMIT 0 ,80";
			$resultado = mysql_query($consulta) or die ('La consulta -faltantes_cuero- fall&oacute;: ' . mysql_error());
            ...
            if (!$resultado)
				...
				while($row = mysql_fetch_array($resultado)){
				  $indice_aux=$contador;
                  $indice_aux=$row['orden_id'];
                  $respuesta[$indice_aux] = $row['faltan'];
                  $contador = $contador + 1;
               }   
            }
            return $respuesta;
    ...
    function ingresar_cueroutilizado($hoja_id,$orden_id,$fecha_corte, $cortador,$cuero_utilizado,$retaceria,$retazos,$desperdicio,$cuero_utilizado_pies,$unidad)
	{
		$con = new DBmanejador;
        if($con->conectar() == true){
			$consulta = "
			INSERT INTO `cuero_utilizado`
			( `hoja_id`,`orden_id`, `fecha_corte`, `cortador`, `cuero_utilizado`, `retaceria`, `retazos`, `desperdicio`, `cuero_utilizado_pies`,`unidad`)
			VALUES
			('".$hoja_id."','".$orden_id."','".$fecha_corte."', '".$cortador."', '".$cuero_utilizado."', '".$retaceria."','".$retazos."','".$desperdicio."', '".$cuero_utilizado_pies."','".$unidad."')
			";
		//	echo $consulta;
			$resultado = mysql_query($consulta) or die ('La consulta -ingresar_cuero_utilizado- fall&oacute;: ' . mysql_error());
		}
	}
	...
	function detalle_orden_cuero($op_id, $detalles_id){
		...
		if($con->conectar() == true){
                    ...
                    SELECT tdop.detalle_id, 
						...
						e.nombre_estilo, 
						top.orden_id, 
						tcuero.descripcion AS cuero, 
						tco.descripcion AS color , 
						h.cantidad, 
						h.hoja_id, 
						tdop.pedido, 
						tdop.cantidad AS cant_orden, 
						tdop.bloqueado, 
						tdop.orden_id 
			...
			$resultado = mysql_query($consulta) or die('La consulta -detalle orden cuero- fall&oacute;: ' . mysql_error());
	...
	function get_order_detail_leather($orderId)
	{
		...
		while($row = mysql_fetch_array($result))
		{
			$query = "
			SELECT cuo.*, 
				CONCAT(p.clase, ' - ', p.apellidos, ' ',p.nombres) AS employee_name 
			FROM cuero_utilizado_orden cuo, personal p
			WHERE order_id = $orderId 
				AND cuo.employee_id = p.personal_id 
				AND leather_id = ". $row['leather_id'] .
				" AND color_id = ". $row['color_id'];
				
			$savedUsedLeather = mysql_query($query) or die('La consulta -get_order_detail_leather- first select fall&oacute;: ' . mysql_error());
    ...
    function verificando_registro($op_id)
    {
		...
		if($con->conectar() == true){
			$consulta = "
				SELECT h.hoja_id 
				FROM hoja AS h, 
					tdetalleordenesproduccion AS tdo 
				WHERE h.detalle_id = tdo.detalle_id 
					AND tdo.orden_id = ".$op_id." 
					AND h.hoja_id 
						NOT IN(SELECT hoja_id FROM cuero_utilizado WHERE orden_id = ".$op_id.")
					";
			
			//echo "<br/>Checking: ".$consulta;
			$resultado = mysql_query($consulta) or die('La consulta -verificando_registro- fall&oacute;: ' . mysql_error());
    ...
    function lista_cuero_utilizados($orden_id)
    {
		...
		if($con->conectar()==true)
		{
            ...
            $consulta="SELECT cu.hoja_id,tdo.orden_id,tdo.pedido,cu.fecha_corte,concat(nombres,' ',apellidos) as cortador,f.nombre_familia,e.nombre_estilo,h.cantidad,tcu.descripcion as cuero,tco.descripcion as color,cu.cuero_utilizado, cu.retaceria,cu.retazos,cu.desperdicio,cu.cuero_utilizado_pies,cu.unidad  
            ...
            order by cu.hoja_id";
			$resultado=mysql_query($consulta) or die('La consulta - lista_cuero_utilizados- fall&oacute;: ' . mysql_error());	
			if (!$resultado) return false;
			else
			{      $cont=0;
					while($row = mysql_fetch_array($resultado))
                    ...
                    $lista[$cont]['orden_id'] = $row['orden_id'];
	...
	//*******************************************************************
    //METODOS PARA Q SALGA LISTA DE MATERIALES EN IMPRESION HOJA DE CORTE
    //*******************************************************************
	 function detalle_corte_rg27($hid){
        $con = new DBmanejador();
        if($con->conectar() == true){
            $consulta = "
                SELECT tmt.descripcion
            ...
            if (!$resultado)
                ...
                while($row = mysql_fetch_array($resultado)){
                    $respuesta[$contador]['nombre'] = $row['descripcion'];
                    $respuesta[$contador]['sis_cantidad'] = $row['cantidad'];
                    $respuesta[$contador]['sis_golpe'] = $row['golpe'];
            ...
            ";
            //echo "<br>La consulta: ".$consulta;
            $resultado = mysql_query($consulta1) or die('La consulta -sacar detalle corte 1- fall&oacute;: ' . mysql_error());
        
            if (!$resultado)
                return false;
            else {
                while($row = mysql_fetch_array($resultado)){
                    $golpe = (int)round(1/$row['golpe']);
                    
                    $respuesta[$contador]['nombre'] = $row['descripcion'];
                    $respuesta[$contador]['sis_cantidad'] = $row['cantidad'];
                    $respuesta[$contador]['sis_golpe'] = $golpe;
                    $contador = $contador + 1;
                }
            }
         sort($respuesta);
	 ...
	 //**************************************************************
    //FIN PARA Q SALGA LISTA DE MATERIALES EN IMPRESION HOJA DE CORTE
    //***************************************************************
