...
class OrdenProd {
    ...
    //***********************************************************************************************************************************
    //                                                  INICIANDO IMPLEMENTACION RG-15 LOCAL
    //***********************************************************************************************************************************
    function consulta_numero_orden_local() {
		$con = new DBmanejador();
		if ($con->conectar() == true) {
            $consulta = '
				SELECT	num_orden
			...
			if (!$resultado)
				...
				$fecha = date("Y");
				if ($fecha == $num_orden[2])
					$num_orden_nueva = "PL_".str_pad(($num_orden[1]+1), 2, "0", STR_PAD_LEFT)."_".$num_orden[2];
				else
					$num_orden_nueva = "PL_5001_".$fecha;
				return $num_orden_nueva;
			}
		}
	}
    
    function obtener_num_cup_local($num_orden) {
		$orden_div = split("_", $num_orden);
		$num_cup = $orden_div[1].substr($orden_div[2], 2, 2);
		return $num_cup;
	}

    function ingresa_nueva_orden_local($num_orden,$num_cup,$fecha,$fecha_entrega,$cliente_id,$observaciones,$usuario_id) {
		$validar = new Validador();
		$fecha1 = $validar->cambiaf_a_mysql($fecha);
		$fecha2 = $validar->cambiaf_a_mysql($fecha_entrega);
		$con = new DBmanejador;
		...
		if ($con->conectar() == true) {
			$consulta = "INSERT  into tordenesproduccion(num_orden,cliente_id,usuario_id,cup_num,fecha,fechaentrega,observacion,tipo)values('".$num_orden."',".$cliente_id.",".$usuario_id.",".$num_cup.",'".$fecha1."','".$fecha2."','".$observaciones."','1')";
			$resultado = mysql_query($consulta) or die('La consulta fall&oacute; en ingresar nueva orden de produccion local: ' . mysql_error());
