...
/* function diferencia_fechas ($fecha_inicio,$fecha_fin)
  {
		...
		//obtener el valor absoulto de los días (quitar el posible signo negativo)
...
return $fecha;
    ...
    /*********************************************************************************************************
    * Objetivo: Actualizar el tipo de cambio desde un archivo excel directamente a la tabla mysql: tipo_cambio 
    * Parametros E: el nombre del archivo siempre debe ser tipos.csv solo esa extension
    *********************************************************************************************************/
function actualizacion_automatica($nombrearchivo)
{   
    $activo= new Activo();
    $row = 1;
    $con = new Conexion;
	if($con->conectar() == true)
    {
		$fp = fopen ($nombrearchivo,"r");//"tipos.csv"
		while ($data = fgetcsv ($fp, 1024, ",")) 
		{ 
			$num = count ($data); 
			$row++; 
			//echo "($row)>>[0]". $data[0]."---[1]".$data[1]."---[2]".$data[2]."<br/>";
			$fecha=date("Y-m-d");
			$fec_ini=date("Y-m-d",strtotime($data[0]));
			 
			if ($fec_ini<=$fecha)//la fecha q se quiere ingresar es mayor q una fecha actual pues las cotizaciones maximo se actulizaran hasta la fecha del mismo dia
			{
				$verificar=$activo->existeFecha($fec_ini,$data[2]);
				if($verificar==false)//q no encontro un valor igual al q se quiere ingresar
				{
					$consulta = "INSERT INTO tipo_cambio(fecha_ini,tipo_cambio,id) VALUES ('".$fec_ini."','$data[1]','$data[2]')";
					//echo "<br>SQL: ".$consulta;			
					$resultado = mysql_query($consulta) or die ('La consulta -actualizar tipo de cambio- fall&oacute;: ' . mysql_error());
					if (!$resultado)
						return false;
				}
				else//true => existe la fecha en tabla tipo cambio
				{
					return $verificar;//cuando existe registro de la fecha y un valor definido para dolar o para UFV
				}
			}
			else
			{
				return "mayor";//cuando existe fecha mayor q la actual
			}
		}
     $activo->actualizar_tipocambio();//funcion q cambia los valore de la fecha final='0000-00-00'	
     fclose ($fp);
	}	
}
    /**********************************************************************************************************************
    * Objetivo: Verificar q no existan valores de dolar o UFV con la misma fecha a ingresar 
    * Parametros E: $fecha, la fecha a buscar en la tabla tipo_cambio ;$i, es el identificador para saber si es dolar o ufv
    ***********************************************************************************************************************/
function existeFecha($fecha,$id)
{
	...
	if($con->conectar() == true){
	$consulta = "
    SELECT * FROM `tipo_cambio` WHERE fecha_ini='".$fecha."' and id='".$id."'
	";
	//echo "<br>SQL: ".$consulta,"<br>";
    ...
    /********************************************************
    * Objetivo: Recuperar los valores de UFV y dolar actuales
    *********************************************************/
...
echo "</pre>";*/
    ...
    /********************************************************************************
    * Objetivo: Actualizar el tipo de dolar y UFV modificando el valor de fecha final
    *********************************************************************************/
function actualizar_tipocambio()
{
	   $activo= new Activo();
       $tipoactuales=$activo->tipoCambioActual();
	   $con = new Conexion;
		if($con->conectar() == true)
        {
            //actualizar los q tienen fecha_fin a NULL
			$consulta = "
			UPDATE	tipo_cambio 
			SET		fecha_fin=NULL				
			WHERE	fecha_fin='0000-00-00'";
			//echo "<br>SQL: ".$consulta,"<br>";		
			$resultado = mysql_query($consulta) or die ('La consulta actualiza fecha_fin- fall&oacute;: ' . mysql_error());
            
            //averiguar cual es la ultima fecha ingresada en tipo de cambio
			$consulta = "SELECT fecha_ini FROM tipo_cambio ORDER BY tipo_id DESC LIMIT 1";
			//echo "<br>La consulta: ".$consulta;
            $resultado = mysql_query($consulta) or die ('La consulta  busca el ultimo registro ingresado- fall&oacute;: ' . mysql_error());
			if ($row = mysql_fetch_array($resultado)) 
            {
				$fecha_ini = $row['fecha_ini'];
			}

			//modificamos resultados de ultima fecha
			$consulta = "
			UPDATE	tipo_cambio
			SET		fecha_fin='0000-00-00'
			WHERE	fecha_ini = '".$fecha_ini."'";
			//echo "<br>La consulta: ".$consulta;
            $resultado = mysql_query($consulta) or die ('La consulta actualiza la fecha final del tipo de cambio- fall&oacute;: ' . mysql_error());
		}		
}	 
	}
